...
async function getdata_reportedistribucion(){
    try{
      let pool =await sql.connect(config);
      let res = await pool.request().query("select isnull(dst_inventario.no_controlE,'') as 'NoControl', isnull(Nom,'') as Customer, CONVERT(varchar, dst_entrada.fecha_llegada,103) as 'ArrivalDate', "+
      "dst_entrada.hora_llegada as 'Time', isnull(dst_cat_lineas.Nombre,'') as Carrier, isnull(dst_entrada.caja,'') as Trailer, "+
      "isnull(dst_inventario.serie,'') as Serial, isnull(dst_inventario.numero_referencia,'') as Skid, isnull(dst_inventario.no_parte,'') as Part, "+
      "isnull(dst_inventario.descripcion,'') as 'Description', "+
      ...
      "where dst_inventario.Cliente = '10012' and dst_entrada.fecha_llegada is not null and cantidad_disponible<>0");
      pool.close();
  ...
  async function getdata_reportemeow(){
    try{
      let pool =await sql.connect(config);
      ...
      "left Join distribucion.dbo.PAQ_DESCB A on A.id =dst_inventario.unidad_bulto "+
      "left join distribucion.dbo.dst_cat_secciones on dst_cat_secciones.ID = dst_inventario.seccion "+
      "where dst_inventario.Cliente = '10009' and dst_entrada.fecha_llegada is not null and cantidad_disponible<>0 ");
      pool.close();
      return res.recordset;
    }
    catch(error){ 
  ...
  async function sp_reporte_distribucion(cliente) {
  try {
    let pool = await sql.connect(config);
    ...
    let resultado = await pool.request()
      ...
      .execute('sp_reporte_distribucion'); 
...
module.exports={
    ...
    getdata_reportemeow:getdata_reportemeow,
    sp_reporte_distribucion:sp_reporte_distribucion,
  }